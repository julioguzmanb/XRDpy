"""Composable motor-axis and diffractometer geometry models.

Transforms use homogeneous ``4 x 4`` matrices and act on row-major point
arrays. Motors may be fixed in the laboratory frame or inherit the moving
local frame generated by preceding motors in a chain.
"""
...
def _as_vector3(vector, name="vector"):
    """Convert an input to a floating three-vector or raise ``ValueError``."""
...
def _normalize_frame(frame):
    """Normalize and validate a motor frame name as ``lab`` or ``local``."""
...
def transform_direction(direction, transform, normalize=True):
    ...
    direction : array-like, shape (3,)
        Direction in the source frame.
    transform : array-like, shape (4, 4)
        Homogeneous source-to-target transform; translation is ignored.
    normalize : bool
        Normalize the transformed direction to unit length.

    Returns
    -------
    numpy.ndarray
        Transformed three-vector.
    """
...
class Motor:
    ...
    def __post_init__(self):
        """Normalize motor fields and validate the axis, origin, and frame."""
    ...
    def resolved_axis_origin(self, parent_transform=None):
        ...
        ``parent_transform`` maps local definitions through preceding motors;
        it is ignored when ``frame="lab"``.
        """
    ...
    def as_axis_rotation(self, angle=None, parent_transform=None):
        ...
        ``angle`` defaults to ``default_angle`` and ``parent_transform``
        resolves a local axis/origin through preceding motors.
        """
        if angle is None:
    ...
    def as_transform(self, angle=None, parent_transform=None):
        ...
        ``angle`` and ``parent_transform`` follow :meth:`as_axis_rotation`.
        """
...
class MotorChain:
    ...
    def from_list(cls, motors, name="chain", metadata=None):
        ...
        ``motors`` is the ordered iterable, ``name`` labels the chain, and
        optional ``metadata`` is copied.
        """
        chain = cls(
            motors=[],
            name=name,
    ...
    def add_motor(self, motor):
        """Append a motor instance or motor dictionary to the chain."""
        if not isinstance(motor, Motor):
            raise TypeError("motor must be an instance of Motor.")
        self.motors.append(motor)

    def extend(self, motors):
        """Append each motor from iterable ``motors`` in application order."""
        for motor in motors:
            self.add_motor(motor)

    @property
    def motor_names(self):
        """Return motor names in chain application order."""
        return [motor.name for motor in self.motors]

    def get_motor(self, name):
        """Return the named motor, raising ``KeyError`` when it is absent."""
    ...
    def default_angles(self):
        """Return a name-to-default-angle mapping for every motor."""
        return {motor.name: motor.default_angle for motor in self.motors}

    def _merged_angles(self, angles=None):
        """Overlay supplied motor angles on the chain defaults."""
    ...
    def as_rotation_chain(self, angles: Optional[Dict[str, float]] = None):
        ...
        ``angles`` overrides defaults by motor name.
        """
    ...
    def as_transform(self, angles: Optional[Dict[str, float]] = None):
        ...
        ``angles`` overrides default motor values by name.
        """
    ...
    def apply(self, points, angles: Optional[Dict[str, float]] = None):
        ...
        ``angles`` optionally overrides default motor values.
        """
...
class DiffractometerGeometry:
    """Pair independent sample and detector motor chains.

    Parameters
    ----------
    name : str
        Human-readable geometry identifier.
    sample, detector : MotorChain
        Ordered motor descriptions for the sample stage and detector arm.
    metadata : dict
        Optional facility- or instrument-specific annotations.
    """
    ...
    def sample_transform(self, angles: Optional[Dict[str, float]] = None):
        """Return the sample-chain transform for the supplied motor angles."""
        return self.sample.as_transform(angles=angles)

    def detector_transform(self, angles: Optional[Dict[str, float]] = None):
        """Return the detector-chain transform for the supplied motor angles."""
    ...
    def transforms(
        ...
        """Return transforms for ``sample_angles`` and ``detector_angles`` maps."""
        return {
